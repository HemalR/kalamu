...
const BLOCK = [
  ...
  "",
  "Placement is part of the record. Before adding, find the branch the node belongs to (`kalamu list`, `kalamu search <term>`) and nest it there with `--parent <id>` — work discovered while doing a task usually belongs under that task or its umbrella; a human-assigned follow-up belongs under the work that raised it. Add at top level only when the node starts a genuinely new area, never because finding the parent takes effort.",
  "",
