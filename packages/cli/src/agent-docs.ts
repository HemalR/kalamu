...
const BLOCK = [
  ...
  "A direct request does not need a node merely because this repository uses Kalamu. Human-invoked workflows may define their own records, including wayfinder discussion tickets; follow that explicit workflow.",
  "",
  ...
  "Claim that discussion with `kalamu start <id>` before discussing it; it leaves `next --discussion` until ended or done. The human-invoked workflow exception above also applies to creating discussion tickets.",
  END_MARKER,
  "",
].join("\n");
