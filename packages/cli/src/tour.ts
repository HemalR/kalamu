  ...
  children: [
    {
      ...
      children: [
        ...
        { kind: "task", priority: 1, tags: ["demo"], text: "Metadata lives in the text: p1…p3 sets priority (p1 = high, p3 = low; the default p2 shows no badge), #tokens become tag chips, / assigns a task to human or agent, and @ opens a repo-file picker" },
    ...
    },
    {
      ...
      children: [
        ...
        { kind: "bullet", text: "The search icon up top (or Cmd/Ctrl+F) opens Find: full-text search across the outline, or paste/type a node id to jump straight to it — press Cmd/Ctrl+F again inside it for the browser's own find" },
        { kind: "bullet", text: "A repo-relative .md path renders as a doc chip — click to read the file, or use its chevron to peek under the row. Add #heading-slug to target a section. Both views show Markdown source; kalamu validate warns if the file is missing" },
        { kind: "bullet", text: "Cmd/Ctrl+Z undoes anything; Cmd/Ctrl+C copies an item's ancestor path and subtree for an agent chat; Cmd/Ctrl+Shift+C copies only its raw text; paste a list into an empty item and each line becomes a sibling of the same kind; paste images straight in — they land in .kalamu/assets/" },
        { kind: "bullet", text: "When done work piles up, Clean up in the palette deletes completed items with their subtrees and can be undone in the UI. The CLI's kalamu clean also removes those subtrees, but has no undo. To delete one subtree, use the row's trash button or Cmd/Ctrl+Shift+Backspace" },
