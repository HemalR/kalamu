/**
 * `kalamu init --wayfinder` scaffolding: the Kalamu issue-tracker doc that
 * the wayfinder planning skill reads (docs/agents/issue-tracker.md) plus a
 * pointer block in the project agent docs so agents know it exists.
 *
 * The tracker doc carries an ownership line: while it is present, re-running
 * init refreshes the file from the template bundled with THIS CLI version;
 * deleting the line hands the file to the repo and stops refreshes. The
 * pointer block uses the same marker-fence upgrade mechanics as the standing
 * Kalamu instruction (agent-docs.ts). The template body is asserted identical
 * to examples/wayfinder/issue-tracker.md by a test — edit the example, then
 * regenerate the lines here.
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { ensureDocBlock } from "./agent-docs.js";

const OWNERSHIP_LINE =
  "<!-- generated by kalamu init --wayfinder; re-running refreshes this file. Delete this line to take ownership and stop refreshes. -->";

/** Identical to examples/wayfinder/issue-tracker.md (enforced by test). */
...
const TRACKER_DOC = `${OWNERSHIP_LINE}\n\n${TRACKER_DOC_BODY}`;
const TRACKER_PATH = join("docs", "agents", "issue-tracker.md");

const POINTER_MARKER = "<!-- kalamu:wayfinder -->";
const POINTER_END = "<!-- /kalamu:wayfinder -->";
const POINTER_BLOCK = [
  POINTER_MARKER,
  "",
  "## Agent skills",
...
/**
 * Write the tracker doc and plant the pointer block. Returns the repo-relative
 * tracker path when the doc was created or refreshed (null when up to date or
 * user-owned) and the agent-doc files the pointer was written to.
 */
export function ensureWayfinderDocs(root: string): { tracker: string | null; pointers: string[] } {
  const path = join(root, TRACKER_PATH);
  let tracker: string | null = null;
  if (!existsSync(path)) {
    mkdirSync(join(root, "docs", "agents"), { recursive: true });
    writeFileSync(path, TRACKER_DOC);
    tracker = TRACKER_PATH;
  } else {
    const content = readFileSync(path, "utf8");
    // The ownership line is the refresh consent; without it the file is the
    // repo's own doc now and init must not touch it.
    if (content.includes(OWNERSHIP_LINE) && content !== TRACKER_DOC) {
      writeFileSync(path, TRACKER_DOC);
      tracker = TRACKER_PATH;
    }
  }
  return { tracker, pointers: ensureDocBlock(root, POINTER_MARKER, POINTER_END, POINTER_BLOCK) };
}
