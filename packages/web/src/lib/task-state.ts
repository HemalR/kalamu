/**
 * Pure helpers for the two task states the outline surfaces beyond done:
 * an agent's claim (`startedAt` — SPEC key decision 17) and what a task waits
 * on (`blockedBy` — key decision 16). No Svelte imports, unit-tested.
 */
...
/**
 * The blockers actually holding the task up, in `blockedBy` order. Done
...
/** Tooltip for the blocked badge: what the task is waiting on, one per line. */
...
/** Open tasks are what a task usually waits on; done nodes are the last resort. */
...
/**
 * What the palette's "Block on…" submenu offers: every other node, minus the
 * ones already recorded. Blockers cross the tree freely (key decision 16), so
 * nothing else is excluded here — a pick that would close a cycle is rejected
 * by core/the server, and the message says why.
 *
...
export interface BlockerEntry {
  id: string;
  label: string;
  /** Done blockers stay listed (they are still recorded) but no longer hold the task. */
  open: boolean;
}
