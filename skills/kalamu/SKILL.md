---
name: kalamu
description: Work with Kalamu, the repo-local task outliner. Use when a repository contains a .kalamu/ directory, or when asked to pick up the next task, record todos, track deferred work, or mark tasks done in a repo that uses Kalamu. Covers getting work (kalamu next), recording work (add, done, handoff), and the rules agents must follow (self tasks, priorities, bullets vs tasks).
license: MIT
...
2. Never work on tasks with `"self": true` (rendered as `(self)`): they belong to the developer. `kalamu next` already excludes them — but they may appear as descendants of a returned task; leave those to the human.
