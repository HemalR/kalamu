...
8. **Assignment: human or agent.** (Amended 2026-07-10: replaces the earlier `self: true` flag.) `assignee: "human"` marks a task the developer keeps for themselves — `next` never returns it. `assignee: "agent"` explicitly marks agent work; omitted means unassigned, which is agent-eligible exactly like `"agent"`. This is still audience, not people: the only two values are the developer at the keyboard and their agents — Kalamu has no concept of users. Legacy `"self": true` reads as `assignee: "human"` and is rewritten on the next write.
   In the command palette, Assign is available on bullets as a promotion shortcut: choosing Human or Agent converts the bullet to a task and assigns it in one operation. Converting any node to a bullet clears its assignee; bullets never retain assignment from an earlier task state. Discussions remain unassignable.
...
Optional ISO timestamp. Meaningful on tasks and discussions; inert on bullets, which cannot be claimed.
...
`node.kind === "discussion"`, minus the one a discussion can never carry:
`assignee` is inert on discussions (never assigned), so only a stale value from
a past life as a task could be present and it never gates. Claims gate
(key decision 17, amended 2026-09-05) and so do blockers (key decision 16,
amended 2026-08-10).
...
Converting any node to `bullet` clears `assignee`. A `task` converted back to `bullet` still preserves `doneAt` and `priority`; those fields are inert on bullets and are restored if the node is converted back to a task. Assignment is not preserved because bullets are not assignable.
...
kalamu next --discussion           # queue discussions instead of tasks: same sort, same
                                   ...
                                   # closed task ancestor (assignee is inert on
                                   # discussions; claims and blockers are not)
```
...
discussion, Assign is disabled (discussions are never assigned — key decision
12); Priority, Toggle done, Start/End, Block, Kind and Copy all apply. The fold actions (`← → ↑`) apply to every kind (they are
...
Clicked from a chat or terminal, every such link would open one more browser tab.
The web manifest therefore declares `launch_handler: { client_mode:
"focus-existing" }` (root-scoped, so one installed app covers every project). Once
the hub is installed as an app and the browser's "open supported links in app"
setting is on, a deep link focuses the existing Kalamu window and arrives via
`launchQueue`: the same project applies the hash (zoom in, or out for an empty
hash), another project is a full navigation. Plain tabs and browsers without
`launchQueue` behave as before — the manifest field is inert there. Opt-in,
per-machine, human-only, like the rest of the hub.
...
* `start` refuses an already-started task without `--force`, succeeds with it; refuses bullets, accepts discussions
