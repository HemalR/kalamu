...
- **Reference a spec doc**: write the repo-relative `.md` path in the node text (`Spec: plans/foo.md Phase 2`), or point into it with a heading slug (`plans/foo.md#phase-2`); the UI renders it as a chip that opens the file at that heading (SPEC key decision 19). `kalamu validate` warns when a referenced file is missing. Prose stays in the doc, state stays in the node — never duplicate one into the other.
...
- **Claim**: `kalamu start <id>` — the session's first write, before any work. Tasks and discussions alike: claiming a discussion tells other sessions the conversation is underway. `kalamu start <id> --force` re-claims a ticket whose session died; `kalamu end <id>` releases a claim without resolving.
...
- **Refer by name**: a ticket's name is the first line of its text; never give the human a bare node id. Cite tickets with the Markdown deep link: `add`/`done`/`start`/`next` echo one on a `Link:` line — paste that, or run `kalamu link <id>` for any other node.
